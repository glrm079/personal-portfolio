...
describe("getTextSegmentMotion", () => {
  ...
  it("reveals text segments upward by default", () => {
    expect(getTextSegmentMotion(false).initial).toEqual({
      opacity: 0,
      y: "0.72em",
    });
...
describe("getHeroDescriptionMotion", () => {
  ...
  it("reveals the description with a blur-to-clear transition by default", () => {
    expect(getHeroDescriptionMotion(false).initial).toEqual({
      opacity: 0,
      y: 18,
      filter: "blur(8px)",
    });
    expect(getHeroDescriptionMotion(false).animate).toEqual({
      opacity: 1,
      y: 0,
      filter: "blur(0px)",
    });
